...
public class DooBuilding : MonoBehaviour
{
    ...
    private void SavePosition()
    {
        ...
        if (key != "")
        {
            PlayerPrefs.SetFloat($"{key}XDooBuilding", position.x);
            PlayerPrefs.SetFloat($"{key}YDooBuilding", position.y);
            PlayerPrefs.SetFloat($"{key}ZDooBuilding", position.z);
            PlayerPrefs.Save();
        }
    }
