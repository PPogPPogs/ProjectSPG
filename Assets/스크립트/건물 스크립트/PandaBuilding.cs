...
public class PandaBuilding : MonoBehaviour
{
    ...
    private void SavePosition()
    {
        ...
        if (key != "")
        {
            PlayerPrefs.SetFloat($"{key}XPandaBuilding", position.x);
            PlayerPrefs.SetFloat($"{key}YPandaBuilding", position.y);
            PlayerPrefs.SetFloat($"{key}ZPandaBuilding", position.z);
            PlayerPrefs.Save();
        }
    }
