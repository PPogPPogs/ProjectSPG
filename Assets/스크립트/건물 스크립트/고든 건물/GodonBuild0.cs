...
public class GodonBuild0 : MonoBehaviour
{
    ...
    private void SavePosition()
    {
        ...
        if (key != "")
        {
            PlayerPrefs.SetFloat($"{key}XGodonBuild0", position.x);
            PlayerPrefs.SetFloat($"{key}YGodonBuild0", position.y);
            PlayerPrefs.SetFloat($"{key}ZGodonBuild0", position.z);
            PlayerPrefs.Save();
        }
    }
    ...
    private void UpGradeDelete()
    {
        ...
        if (key != "")
        {
            ...
            PlayerPrefs.DeleteKey($"{key}XGodonBuild0");
            PlayerPrefs.DeleteKey($"{key}YGodonBuild0");
            PlayerPrefs.DeleteKey($"{key}ZGodonBuild0");
            PlayerPrefs.Save();

        }
        Destroy(GodonBuild0Prefab);
        Instantiate(GodonBuild1Prefab, transform.position, Quaternion.identity);
    }
