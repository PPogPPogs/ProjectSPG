...
public class GodonBuild1 : MonoBehaviour
{
    ...
    private void SavePosition()
    {
        ...
        if (key != "")
        {
            PlayerPrefs.SetFloat($"{key}XGodonBuild1", position.x);
            PlayerPrefs.SetFloat($"{key}YGodonBuild1", position.y);
            PlayerPrefs.SetFloat($"{key}ZGodonBuild1", position.z);
            PlayerPrefs.Save();
        }
    }
    ...
    private void UpGradeDelete()
    {
        ...
        if (key != "")
        {
            ...
            PlayerPrefs.DeleteKey($"{key}XGodonBuild1");
            PlayerPrefs.DeleteKey($"{key}YGodonBuild1");
            PlayerPrefs.DeleteKey($"{key}ZGodonBuild1");
            PlayerPrefs.Save();

        }
        Destroy(GodonBuild1Prefab);
        Instantiate(GodonBuild2Prefab, transform.position, Quaternion.identity);
    }
